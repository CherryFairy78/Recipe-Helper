...
public sealed class AetherialReductionService
{
    ...
    public double GetAvailabilitySortKey(
        uint itemId,
        IReadOnlyList<AetherialReductionSource>? reductionSources)
    {
        var currentMinute = GetCurrentEorzeaMinute();
        if (reductionSources is { Count: > 0 })
        {
            var preferredSource = this.GetPreferredSource(reductionSources);
            return preferredSource is { IsFishing: false, Windows.Count: > 0 }
                ? GetWaitMinutes(preferredSource, currentMinute)
                : 2001d;
        }
        ...
        var timedSource = new AetherialReductionSource(
            itemId,
            string.Empty,
            false,
            windows);
